...
- **`/mcp`** - an MCPServer **Streamable HTTP** server (`infrastructure/web/mcp_adapter.py`), mounted on the FastAPI app and run from the app lifespan. An exact slashless route forwards internally to the canonical mounted app, so the advertised URL works through an HTTPS tunnel without a proxy-derived redirect. The SDK's static localhost Host policy is replaced by the outer per-launch capability boundary, which runs before MCP dispatch. Tools: `run_command`, `read_screen`, `send_keys`, `send_signal`.
- **`/api/agent/*`** - REST endpoints for agents that can make HTTP requests but cannot register an MCP server: `run`, `screen`, `keys`, `signal`, and `session` close. Only `run` without an ID creates a REST session; every ID-bearing operation requires a live server-issued ID and otherwise returns 404 without creating terminal state.
- **`AgentTerminalService`** (`application/services/agent_terminal_service.py`) maps one MCP session or REST `session_id` to a dedicated PTY session + 🤖 tab. MCP sessions reap on client disconnect; REST sessions reap on explicit close, shell exit, or idle cleanup, and closed IDs cannot recreate them.
