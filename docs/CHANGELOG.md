...
- **False "Copied" on GNOME Terminal** - the OSC 52 fallback is skipped in
  ...
  terminal acknowledges OSC 52. When no clipboard tool is installed on a Linux
  desktop, the message now says which package to install.

### Changed

- `porterminal.cli.copy_to_clipboard()` returns a `CopyResult`
  (`COPIED` / `SENT_TO_TERMINAL` / `UNAVAILABLE`) instead of a bool; only
  `COPIED` is truthy. `PORTERMINAL_DISABLE_OSC52_CLIPBOARD=1` (existing opt-out)
